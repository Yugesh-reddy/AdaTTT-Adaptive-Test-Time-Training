...
from ttt.data import (
    VQADataset, CachedFeaturesDataset,
...
def main():
    ...
    parser.add_argument("--features", type=str, default=None,
                        help="Optional path to precomputed features .pt "
                             "(from gpu/precompute_features.py). Skips ViT+BERT "
                             "re-encoding.")
    ...
    # Optionally wrap with cached features to skip frozen-encoder forwards.
    # Rotation re-encodes rotated images; consistency re-encodes augmented views → need live images.
    use_features = args.features is not None
    needs_images = args.objective == "rotation" or mode_config["use_consistency"]
    if use_features:
        val_dataset = CachedFeaturesDataset(
            features_path=args.features,
            source_dataset=val_dataset if needs_images else None,
            load_images=needs_images,
        )
        collate_fn = cached_vqa_collate_fn
        logger.info(
            f"Using cached features: {args.features} "
            f"(load_images={needs_images} for mode={args.mode}/obj={args.objective})"
        )
    else:
    ...
    for batch in tqdm(val_loader, desc=f"Ablation {args.mode} K={args.k}"):
        ...
        for i in range(B):
            ...
            sample_images = images[i:i + 1] if images is not None else None
...
if __name__ == "__main__":
    main()
# Improve ablation runner logging output
