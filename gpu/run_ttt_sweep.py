...
def main():
    ...
    parser.add_argument("--features", type=str, default=None,
                        help="Optional path to precomputed features .pt "
                             ...
                             "re-encoding, ~6x sweep speedup on H100.")
    ...
    # Rotation TTT re-encodes rotated images → needs live images.
    use_features = args.features is not None
    needs_images = objective == "rotation"
    if use_features:
        ...
        logger.info(
            ...
            f"(load_images={needs_images} for objective={objective})"
        )
    else:
    ...
    for batch in tqdm(val_loader, desc=f"TTT K={k} ({objective})"):
        ...
        if use_features:
            ...
            # Images only present when load_images=True (rotation objective)
...
if __name__ == "__main__":
    main()
# Add early stopping to TTT sweep runner
