...
def compute_avg_flops(
    skip_count: int,
    adapt_count: int,
    k_steps: int,
    use_consistency: bool = False,
) -> float:
    """Route FLOP reporting through AdaptiveRouter's canonical helper."""
    router = AdaptiveRouter.__new__(AdaptiveRouter)
    routing_info = {
        "skip_count": int(skip_count),
        "adapt_count": int(adapt_count),
    }
    return AdaptiveRouter.compute_flops(
        router,
        routing_info,
        k_steps=k_steps,
...
def main():
    ...
    if os.path.exists(base_path):
        ...
        ci = bootstrap_confidence_interval(preds, gts)
        avg_flops = compute_avg_flops(
            skip_count=len(base_data),
            adapt_count=0,
            k_steps=0,
        )

        print(f"\n📊 Base Model (no TTT)")
        print(f"   Accuracy: {ci['accuracy']*100:.2f}% "
              f"(95% CI: [{ci['ci_lower']*100:.2f}%, {ci['ci_upper']*100:.2f}%])")
        ...
        all_results.append({
            ...
            "accuracy_ci_lower": ci["ci_lower"],
            "accuracy_ci_upper": ci["ci_upper"],
            "accuracy_ci_width": ci["ci_width"],
            "avg_flops": avg_flops,
    ...
    if os.path.exists(ttt_dir):
        ...
        for ttt_file in ttt_files:
            ...
            for qt, a in type_acc.items():
                print(f"   {qt}: {a*100:.2f}%")
            print(f"   Est. GFLOPs/sample: {avg_flops:.1f}")
    ...
    for af in adaptive_files:
        ...
        # Current result files do not persist consistency metadata, so analysis
        # treats these configurations as non-consistency runs for FLOP reporting.
        avg_flops = compute_avg_flops(
            skip_count=skip,
            adapt_count=adapt,
            k_steps=k,
...
# Add final Memotion2 cross-task result notes
