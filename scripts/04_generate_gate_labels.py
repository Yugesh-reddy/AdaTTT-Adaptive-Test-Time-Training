...
def main():
    ...
    for sample_id in base_map:
        ...
        if ttt_hurts:
            ...
            stats["neither"] += 1
            stats["total"] += 1
            continue  # drop ambiguous samples — not soft-labeled
    ...
    print(f"  Neither (dropped):         {stats['neither']:6d}  ({100*stats['neither']/max(stats['total'],1):.1f}%)")

    # Coverage stats
    kept = len(gate_labels)
    n_pos = sum(1 for gl in gate_labels if gl["gate_label"] == 1.0)
    n_neg = sum(1 for gl in gate_labels if gl["gate_label"] == 0.0)
    print(f"\nCoverage:")
    print(f"  Total evaluated: {stats['total']}")
    print(f"  Kept: {kept} ({100*kept/max(stats['total'],1):.1f}%)")
    print(f"  Dropped (neither): {stats['neither']}")
    print(f"  SKIP (1.0): {n_pos}, ADAPT (0.0): {n_neg}")
    if n_pos + n_neg > 0:
        print(f"  SKIP fraction: {n_pos/(n_pos+n_neg)*100:.1f}%")
