...
class TestMaskedPatchUsesSequence:
    def test_masked_patch_uses_return_sequence(self, config, model_and_tensors):
        """Fusion must be called with return_sequence=True for per-token reconstruction."""
        model, images, visual, text, text_mask = model_and_tensors
        adapter = TTTAdapter(model, config, objective="masked_patch", k_steps=1)
        ...
        class Spy:
            ...
            def __call__(self, v, t, m, return_sequence=False):
                calls.append(return_sequence)
                return self.inner(v, t, m, return_sequence=return_sequence)

            def __getattr__(self, name):
                return getattr(self.inner, name)

        object.__setattr__(model, 'fusion', Spy(original))
        try:
            adapter.masked_patch_loss(visual, text, text_mask)
        finally:
            model.fusion = original
        assert any(calls), "fusion must be called with return_sequence=True"


class TestTrainModeAuxHeads:
    def test_train_keeps_aux_heads_in_eval(self, config):
        """Auxiliary TTT heads stay in eval mode when model.train() is called."""
        model = FullVQAModel(config)
        model.train()
        assert model.fusion.training is True
        assert model.mask_proj.training is False
        assert model.rotation_head.training is False
