...
class TestBootstrapCI:
    def test_perfect_zero_width(self):
        """Perfect predictions → CI width is 0."""
        ci = bootstrap_confidence_interval([0, 1, 2], [0, 1, 2])
        assert ci["accuracy"] == 1.0
        assert ci["ci_width"] == 0.0

    def test_contains_point(self):
        """CI contains the point estimate."""
        ci = bootstrap_confidence_interval([0, 1, 2, 0, 1], [0, 1, 0, 0, 1])
    ...
    def test_tighter_with_more_samples(self):
        ...
        import random
        random.seed(0)
        sp = [random.randint(0, 3) for _ in range(50)]
        sg = [random.randint(0, 3) for _ in range(50)]
        lp, lg = sp * 20, sg * 20
        assert (bootstrap_confidence_interval(lp, lg)["ci_width"]
                < bootstrap_confidence_interval(sp, sg)["ci_width"])

    def test_empty(self):
        """Empty input → accuracy = 0.0."""
        ci = bootstrap_confidence_interval([], [])
        assert ci["accuracy"] == 0.0
