...
def bootstrap_confidence_interval(
    ...
    Returns:
        Dict with accuracy, ci_lower, ci_upper, ci_width.
    """
    if not predictions:
        return {"accuracy": 0.0, "ci_lower": 0.0, "ci_upper": 0.0, "ci_width": 0.0}
    ...
    for i in range(n_bootstrap):
        ...
        accs[i] = (preds[idx] == gts[idx]).mean()

    alpha = 1 - confidence_level
    lo = float(np.percentile(accs, 100 * alpha / 2))
    hi = float(np.percentile(accs, 100 * (1 - alpha / 2)))
    point = float((preds == gts).mean())

    return {"accuracy": point, "ci_lower": lo, "ci_upper": hi, "ci_width": hi - lo}
